...
contract Crowdfund is Ownable {
    ...
    uint256 public constant ONE_DAY = 15 minutes;//for dev
    uint256 public restartWaitTime = 15 minutes;
    ...
    //Distribution rate from dividendPool, 1%
    ...
    struct CrowdfundInfo {
        ...
        uint256 withdrawTotalAmount; //after completed ,when user withdarw the income,accumulate user's compoundAmount
    }
    ...
    event CrowdfundCompleted(uint256 indexed _curRestartTimes,uint256 indexed _curRoundNum);
    ...
    constructor(
        ...
        //the mapping of delete user will point index 0
    ...
    function initNewCrowdfund(uint256 addAmount) internal {
        ...
        if(schedulePoolInfo.curTotalAmount > 0) {
            ...
            if(userList.length > 1){
                initIndex = userList.length -1 + addAmount;
            }
        }
    ...
    */
    function invest(uint256 _amount) public returns(bool) {
        ...
        require(isContinued, 'failed now, please wait restart!');
        require(userIndex[msg.sender] == 0,'cannot invest when user already existed');
    ...
    function batchJoinCrowdfundFromSchedule() public returns(bool) {
        if(isNeedBatch) {
            CrowdfundInfo storage curCrowdfund = allCrowdfunds_[curRestartTimes][curRoundNum];
    ...
    function isCompleted() internal {
        ...
        if(curCrowdfund.curTotalAmount >= maxLimitPerRounds[curRoundNum]) {
            ...
            emit CrowdfundCompleted(curRestartTimes,curRoundNum);
            if(curRoundNum == 1) {
                allTotalAmount[curRoundNum] = curCrowdfund.curTotalAmount;
            }else {
                allTotalAmount[curRoundNum] = allTotalAmount[curRoundNum -1].add(curCrowdfund.curTotalAmount);
            }
        }
    }
     ...
     * @return true when success
    */
    function refund() public onlyJoined returns(bool) {//todo chongxie
    ...
    */
    ...
    function withdraw() public onlyJoined returns(bool) {
        User memory crowdfundUser = users[_msgSender()];
        uint256 _investAmount = crowdfundUser.investAmount;
        require(crowdfundUser.investStatus == uint8(1),'user should be crowdfund status');
        require(userIndex[_msgSender()] != 0,'user should be in invested status!');
        ...
        CrowdfundInfo memory joinedCrowdfund =  allCrowdfunds_[_joinedRestartTime][_joinedRoundNum];
    ...
    function reinvestJoinSchedule() public onlyJoined returns(bool) {
        ...
        CrowdfundInfo memory joinedCrowdfund =  allCrowdfunds_[_joinedRestartTime][_joinedRoundNum];
        bool _can = _canWithdraw(_msgSender());
        require(_can,"reinvestJoinSchedule: can not withdraw");

        joinedCrowdfund.withdrawTotalAmount = joinedCrowdfund.withdrawTotalAmount.add(_investAmount);
        uint256 _income = _investAmount.mul(staticRate).div(100);
        ...
        //start schedule pool mint //new need of stayed same investAmount
        _joinScheduleMint(schedulePoolId, _msgSender(), _investAmount);
        //distribute 8% of income
        safeTokenTransfer(_msgSender(), _income);
        totalWithdraw = totalWithdraw.add(_income);

        //update schedulePool info
        updateSchedulePoolInfo(lastIndex,_investAmount);
    ...
    */
    function cancelSchedule() public onlyJoined returns(bool) {
        User memory scheduleUser = users[_msgSender()];
        ...
        scheduleUser.investAmount = 0;
        scheduleUser.investStatus = uint8(0);
        if(scheduleUser.joinRound > 0) {
            scheduleUser.joinRound = 0;
        }
        userIndex[_msgSender()] = 0;
        //update new user status to schedule
    ...
    function setMaxLimitPerRound(uint256 _start,uint256 _end) public returns(bool) {
        require(_start >= 100 && _end.sub(_start) <= 100,'index wrong');
